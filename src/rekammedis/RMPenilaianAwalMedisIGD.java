...
public final class RMPenilaianAwalMedisIGD extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        TglAsuhan.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "18-06-2024 14:38:54" }));
    ...
    private void isRawat() {
        try {
            ...
            try {
                ...
                // Check if there is radiology result
                if (Sequel.cariInteger("select count(no_rawat) from hasil_radiologi where no_rawat='" + TNoRw.getText() + "'") > 0) {
                    Radiologi.setText(Sequel.cariIsi("select hasil from hasil_radiologi where no_rawat=?", TNoRw.getText()));
                }
                    // Check if there are lab results
                if (Sequel.cariInteger("select count(no_rawat) from detail_periksa_lab where no_rawat='" + TNoRw.getText() + "'") > 0) {
                    ...
                    try {
                        psLab = koneksi.prepareStatement(
                                ...
                                "where detail_periksa_lab.no_rawat = ? and (detail_periksa_lab.tgl_periksa like ? or template_laboratorium.Pemeriksaan like ?) " +
                                "order by detail_periksa_lab.tgl_periksa, detail_periksa_lab.jam");
                        psLab.setString(1, TNoRw.getText());
                        psLab.setString(2, "%");  // assuming you want to match any date
                        psLab.setString(3, "%");  // assuming you want to match any pemeriksaan

                        rsLab = psLab.executeQuery();
                        StringBuilder labResults = new StringBuilder();
                        while (rsLab.next()) {
                            labResults.append("Pemeriksaan : ").append(rsLab.getString("Pemeriksaan"))
                                       .append(" : ").append(rsLab.getString("nilai"))
                                      .append("\n");
                        }
                        Laborat.setText(labResults.toString());
                    } catch (Exception e) {
                        System.out.println("Notif Lab : " + e);
                    } finally {
                        if (rsLab != null) {
                            rsLab.close();
                        }
